...
int main_file(void)
{
	...
	//파일에 쓰기
	/*
	FILE* file = fopen("C:\\Users\\shin\\Documents\\coding\\test1.txt","wb"); // r 읽기전용 w 쓰기전용 a 이어쓰기 
											  // t text     b binaryData
	if (file == NULL)
	{
		printf("파일 열기 실패 \n");
	...
	*/
	...
	//파일 읽기
	/*
	FILE* file = fopen("C:\\Users\\shin\\Documents\\coding\\test1.txt", "rb"); // r 읽기전용 w 쓰기전용 a 이어쓰기 
											  // t text     b binaryData
	if (file == NULL)
	{
		printf("파일 열기 실패 \n");
	...
	*/
	...
	fscanf(file, "%s %d\n", str2, &bonus); // 빈 배열값을 넣어줌으로써, 파일안에있는 텍스트를 넣을 수 있음
	printf("%s %d \n", str2,  bonus);
