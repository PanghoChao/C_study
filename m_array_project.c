...
int arrayAnimal[4][5]; //20장 카드 지도
int checkAnimal[4][5]; //뒤집혔는지 여부 확인
char* strAnimal[10];
char* cPtr = "테스트";

int getEmptyPosition(); //	빈공간 찾기
...
int main_m_array_project(void)
{
	...
	while (1)
	{
		...
		{
			printf("\n\n 축하합니다 ! 모든 동물을 다 찾았네요 \n");
			printf("지금까지 총 %d번 실수 하였습니다.", failCount);
			break;
		}
		...
		else if (checkAnimal[firstSelect_x][firstSelect_y] == 0
				&& checkAnimal[secondSelect_x][secondSelect_y] == 0

				&&
				arrayAnimal[firstSelect_x][firstSelect_y]
				== arrayAnimal[secondSelect_x][secondSelect_y])//카드가 뒤집히지 않았는지&& 두 동물이 같은지?
			{
...
void printQusetion() //문제 출력 (카드 지도)
{
	printf("\n\n(문제)\n");
	int seq = 0;
	//                 seq					checkAnimal
